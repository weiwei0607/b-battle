...
import { Store, Zap, ShieldCheck, AlertTriangle, Swords, Globe } from 'lucide-react';
...
const Header = ({ currentTier, coins, debt, onShopClick, setView, willpowerExp, lang, setLang }) => {
  ...
  const langLabel = { zh: '繁中', en: 'EN', ja: '日本語' };

  return (
    <header className="flex justify-between items-center z-10 py-6 shrink-0 gap-2">
      ...
      <div className="flex gap-1.5 items-center shrink-0">
        ...
        <button 
          onClick={toggleLang}
          className="bg-stone-100 hover:bg-stone-200 text-stone-500 px-2 py-2 rounded-xl flex items-center gap-1 transition-colors border border-stone-200"
        >
          <Globe size={12} />
          <span className="text-[9px] font-black">{langLabel[lang]}</span>
        </button>
