...
content = content.replace(/(\s*\/\/ 親密度)/g, zhAdditions + '$1');
const enBlockRegex = /(en: \{.*?)(\s*\/\/ Bond)/s;
content = content.replace(enBlockRegex, '$1' + enAdditions + '$2');
const jaBlockRegex = /(ja: \{.*?)(\s*\/\/ Bond)/s;
content = content.replace(jaBlockRegex, '$1' + jaAdditions + '$2');
