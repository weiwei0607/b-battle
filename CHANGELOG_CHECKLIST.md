...
* **2026-04-13**: **核心視覺與對戰平衡重大更新**：
    - **羅馬柱碎裂視覺 (Cracked Pillar)**：實裝 SVG 動態裂痕。HP < 30% 出現裂縫，HP < 15% 裂縫加粗、震動 (`pillar-shake`) 且頂部轉紅。
    - **傷害計算邏輯統整**：`getDmgPercent` 現在與動態預算池 (`weeklyPools`/`monthlyPools`) 完全同步，修正 Log 與 UI 比例不一的 Bug。
    - **戰鬥日誌 (Battle Log) 視覺分級**：自動識別傷害量並標註 `[擦傷]`、`[受損]`、`[致命!!]` (含脈衝動畫)。
    - **跨幣別公平性**：對戰數據改為百分比 (HP%) 制，徹底解決 TWD vs JPY 的幣別戰損不公問題。
    - **Prime 會員功能恢復**：意志殿堂恢復金色漸層按鈕，解鎖自定義靈魂 (Custom Persona) 入口。
    - **成就系統修正**：對齊 localStorage 版本，修正 `KARMA_MASTER` 刪除計數失效問題。

* **2026-04-13**: **意志力戰線最終擴張 (Master Update)**：
    - **Combo Mode 連勝獎勵**：連續 3 天低消觸發。金色羅馬柱特效、雙倍金幣回饋、AI 崇拜模式。
    - **Streak Broken 特效**：連勝中斷觸發「灰階霧化」全螢幕 2.8 秒懲戒。
    - **願望清單進度條**：意志殿堂新增目標進度視覺化，達成時觸發慶祝特效。
    - **Firestore 數據遷移**：History 遷移至 `users/{uid}/history` 子集合，徹底解決 1MB 儲存上限風險。
    - **數據衝突解決**：實裝 `updatedAt` 雙向寫入與版本比對邏輯，確保雲端/離線數據安全。
    - **Bug 修復與拋光**：修復 Splash 缺失導致的白屏、統一全域 icon 為 Trophy、AchievementModal 語系化、PWA 自動更新實裝。

* **2026-04-13**: **連勝獎勵與金色守護模式實裝**：
    - **連勝機制 (Saving Streak)**：連續 3 天戰損低於 200 時觸發。
    - **金色羅馬柱 (Golden Pillars)**：連勝期間柱子獲得雙層金色發光效果 (`combo-glow`) 與金色內飾。
    - **人格崇拜模式**：AI 對話框變為金屬質感，台詞轉為誇獎與崇拜模式（由 Gemini 動態調整）。
    - **雙倍金幣加成**：連勝期間記帳獲得的金幣翻倍，強化省錢動力。
    - **Streak Broken 特效**：連勝中斷時觸發全螢幕「灰階霧化」警報，強制停頓 2.8 秒以示懲戒。

* **2026-04-13**: **UX 與數據同步系統強化**：
    - **願望清單進度條**：意志殿堂新增金幣目標設定，進度達成時觸發慶祝特效。
    - **數據同步衝突解決**：實裝 `updatedAt` 雙向寫入邏輯，確保離線資料不被舊雲端覆蓋，強化資料一致性。
