...
const zhAcLines = achievements.map(a => `    ac_${a.id}_name: '${a.zhName}', ac_${a.id}_desc: '${a.zhDesc}',`).join('\n');
const enAcLines = achievements.map(a => `    ac_${a.id}_name: '${a.enName}', ac_${a.id}_desc: '${a.enDesc}',`).join('\n');

const localesPath = path.join(__dirname, 'src/utils/locales.js');
let content = fs.readFileSync(localesPath, 'utf8');

// Update all achievement sections
const zhRegex = /\/\/ --- 征途系列 ---.*?achievement_journey: '征途：成長紀錄'/s;
content = content.replace(/\/\/ --- Achievements ---\n.*?bond_0: '陌生人'/s, (m) => {
  return '// --- Achievements ---\n' + zhAcLines + '\n    // 親密度\n    bond_0: \'陌生人\'';
});

content = content.replace(/achievement_forbidden: 'Forbidden: Secret',.*?bond_0: 'Stranger'/s, (m) => {
  return "achievement_forbidden: 'Forbidden: Secret',\n" + enAcLines + "\n    // Bond\n    bond_0: 'Stranger'";
});

content = content.replace(/achievement_forbidden: '禁忌：秘密',.*?bond_0: '未見の者'/s, (m) => {
  return "achievement_forbidden: '禁忌：秘密',\n" + enAcLines + "\n    // Bond\n    bond_0: '未見の者'";
});

fs.writeFileSync(localesPath, content, 'utf8');
console.log("Locales fixed with correct keys in all blocks.");

// Update Sync logic to save updatedAt to local
const syncPath = path.join(__dirname, 'src/hooks/useFirebaseSync.js');
let syncContent = fs.readFileSync(syncPath, 'utf8');
syncContent = syncContent.replace(
  /updatedAt: Date\.now\(\),/g,
  "updatedAt: Date.now(),\n        }, { merge: true }).then(() => save('updatedAt', Date.now()));"
);
// Fix double closing bracket if any
syncContent = syncContent.replace(/\}\), \{ merge: true \}\)\.then/g, "}, { merge: true }).then");

fs.writeFileSync(syncPath, syncContent, 'utf8');
console.log("FirebaseSync fixed with local timestamp update.");
